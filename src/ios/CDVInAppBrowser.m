    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (weakSelf.inAppBrowserViewController != nil) {
            if (weakSelf.viewController.navigationController) {
                [weakSelf.viewController.navigationController pushViewController:_inAppBrowserViewController animated:NO];
            } else {
...
- (void)hide:(CDVInvokedUrlCommand*)command
{
    ...
    if (weakSelf.inAppBrowserViewController != nil) {
        _previousStatusBarStyle = -1;
        if (weakSelf.viewController.navigationController) {
            [weakSelf.inAppBrowserViewController.navigationController popViewControllerAnimated:NO];
        } else {
            [weakSelf.inAppBrowserViewController.presentingViewController dismissViewControllerAnimated:NO completion:nil];
        }
    }
